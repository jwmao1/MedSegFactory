...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if dual_attn:
            self.to_q_mask = linear_cls(query_dim, self.inner_dim, bias=bias)
            self.to_k_mask = linear_cls(query_dim, self.inner_dim, bias=bias)
            self.to_v_mask = linear_cls(query_dim, self.inner_dim, bias=bias)
...
class DualAttnProcessor:
    ...
    def __call__(
        ...
        # print(f'hidden_states:{hidden_states.shape}, img:{img_hidden_states.shape}, mask:{mask_hidden_states.shape}')
        
        img_mask, mask_mask = None, None
        
        if attention_mask is not None:
            img_mask, mask_mask = attention_mask.chunk(2, dim=0)
        
        # print(mask_hidden_states.shape, img_hidden_states.shape)
        ...
        # img_query = img_query.view(int(batch_size // 2), -1, attn.heads, head_dim).transpose(1, 2)

        # img_key = img_key.view(int(batch_size // 2), -1, attn.heads, head_dim).transpose(1, 2)
        # img_value = img_value.view(int(batch_size // 2), -1, attn.heads, head_dim).transpose(1, 2)
        
        img_query = attn.head_to_batch_dim(img_query)
        img_key = attn.head_to_batch_dim(img_key)
        img_value = attn.head_to_batch_dim(img_value)
        ...
        img_hidden_states = attn.batch_to_head_dim(img_hidden_states)

        # the output of sdp = (batch, num_heads, seq_len, head_dim)
        # TODO: add support for attn.scale when we move to Torch 2.1
        # img_hidden_states = F.scaled_dot_product_attention(
        #     img_query, img_key, img_value, attn_mask=img_mask, dropout_p=0.0, is_causal=False
        # )
        # img_hidden_states = img_hidden_states.transpose(1, 2).reshape(int(batch_size // 2), -1, attn.heads * head_dim)
        # img_hidden_states = img_hidden_states.to(img_query.dtype)

        # for ip-adapter
        # print(self.to_q_mask.weight.device, img_hidden_states.device)
        ...
        mask_query = attn.head_to_batch_dim(mask_query)
        mask_key = attn.head_to_batch_dim(mask_key)
        mask_value = attn.head_to_batch_dim(mask_value)

        # mask_query = mask_query.view(int(batch_size // 2), -1, attn.heads, head_dim).transpose(1, 2)
        # mask_key = mask_key.view(int(batch_size // 2), -1, attn.heads, head_dim).transpose(1, 2)
        # mask_value = mask_value.view(int(batch_size // 2), -1, attn.heads, head_dim).transpose(1, 2)
        ...
        mask_hidden_states = attn.batch_to_head_dim(mask_hidden_states)

        # the output of sdp = (batch, num_heads, seq_len, head_dim)
        # TODO: add support for attn.scale when we move to Torch 2.1
        # mask_hidden_states = F.scaled_dot_product_attention(
        #     mask_query, mask_key, mask_value, attn_mask=mask_mask, dropout_p=0.0, is_causal=False
        # )
        with torch.no_grad():
            self.mask_attn_map = mask_attention_probs
            self.img_attn_map = img_attention_probs
            # print(self.attn_map.shape)

        # mask_hidden_states = mask_hidden_states.transpose(1, 2).reshape(int(batch_size // 2), -1, attn.heads * head_dim)
        # mask_hidden_states = mask_hidden_states.to(mask_query.dtype)
